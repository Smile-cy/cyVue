...
background-size：     规定背景图片的尺寸。
	...
	☞ contain        将背景图片按照原来的缩放比，完整的显示到容器中【不确定是否会将整个容器占满】
```
...
```CSS
  ☞ 线性渐变：
	   ...
	   background-image:  linear-gradient(
                to right, 或者多少度【135deg】
                red,
                blue
            }
	
	例如： background-image:  linear-gradient(
                135deg,
                red 20%,
                blue 40%,
  ...
  ☞ 径向渐变：
		   /* 径向渐变 */
			background-image: radial-gradient(
				 100px at center,【在我们的盒子中心有个100像素的圆】
				 red,
				 blue
			);
```
...
```css
  ...
  ☞ transform-style: preserve-3d;
	 将平面图形转换为立体图形
```
css中加入透视,在网页中实现近大远小[取600-1000]：perspective:1000px;
