...
```javascript
 v-text 只能用在双标签中【一定要有个根标签】
 v-text 其实就是给元素的innerText赋值【直接显示内容】
 v-html 其实就是给元素的innerHTML赋值【能插标签，并且显示出来】
...
<script>
  ...
  //局部组件：声子  挂子   用子
    ...
    var Vcontent =  {
      template:`<div class="content">我是内容组件<Vbtn/></div>`
    }
  //1.创建局部组件先得----声明局部组件【首字母大写】
  var App = {
      template:`
      <div class="wrapper">
        <Vheader />
